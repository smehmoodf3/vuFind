...
{	//, 'ItemDetails.Model' , ItemDetails
	...
	,	events: {
		...
		,   'click [data-action="addtocarttest"]' : 'addToCartClickEvent'
		,	'click [data-type="add-to-cart"]': 'customAddToCart'
		,	'click [data-toggle="set-option"]' : 'custSetOption'
		,	'change [data-toggle="select-option"]': 'custSetOption'
		, 	'click': 'closeDropDownIfPresent'
		}
	, closeDropDownIfPresent: function(e)
		{
			//if(!jQuery(e.target).data() || !jQuery(e.target).data('id') || (jQuery(e.target).data('id').indexOf('add_to_cart') == 1))
			if(jQuery(e.target).closest('div[id$=_add_to_cart]').length == 0)
			{
				if(jQuery(e.target).data('action') !== "addtocarttest"){
					jQuery("[id$=_add_to_cart]").hide();
				}
			}
		}
	,	custSetOption: function(e)
		{
			e.preventDefault();
			var id = jQuery(e.target).parent().parent().data('id')
			,	model = this.model.get('items').get(id)
			,	value = jQuery(e.target).val() || $target.data('value') || null
			,	cart_option_id = jQuery(e.target).closest('[data-type="option"]').data('cart-option-id');
			model.customSetOption(cart_option_id, value);
			if(model.getOption('custcol_gen_color') && model.getOption('custcol_gen_size'))
			{
				jQuery('button[data-id="'+id+'"]').removeAttr('disabled');
			}
			else
			{
				jQuery('button[data-id="'+id+'"]').attr('disabled','disabled');
			}
	...
	,	addToCartClickEvent : function(e)
		{	
			jQuery("#"+ jQuery(e.target).data('id')).toggle();
		}	
	,	initialize: function (options)
		{
	...
	, 	customAddToCart : function (e) 
		{
		jQuery("[id$=_add_to_cart]").hide();
			e.preventDefault();
			var id = jQuery(e.target).data('id')
			,	value = jQuery("#quantity_"+id).val()
			,	model = this.model.get('items').get(id)
			,	self = this
			...
			model.customSetOption('quantity', value);
			if (model.cartItemId)
			{
...
define('ItemDetails.Model', ['ItemOptionsHelper'], function (ItemOptionsHelper)
{
	...
	,	customSetOption: function (option_name, value, dont_validate)
		{
			...
			else
			{
				...
				var option = this.customGetPosibleOptionByCartOptionId(option_name) || this.customGetPosibleOptionByCartOptionId(option_name.toLowerCase());
    ...
    ,    getCustomDetails:function(){
            ...
            xmlhttp.open('POST', _.getAbsoluteUrl('services/service.ss'), false);
...
define('ItemDetails.Router', [], function ()
{
    ...
    return Backbone.Router.extend({
        ...
        ,	initialize: function (options)
        {
            ...
            this.PersonalNoteModel = options.personalNoteModel;
        ...
        ,	productDetails: function (api_query, base_url, options)
        {
            ...
            // we create a new instance of the ProductDetailed View
                ,   personalNoteModel = new this.PersonalNoteModel({personalNote: ''}, {validate: true})
                ,	view = new this.View({
                    model: model
                    ,	personalNoteModel: personalNoteModel
                    ,	baseUrl: base_url
...
define('ItemDetails.View', ['Facets.Translator', 'ItemDetails.Collection'], function (FacetsTranslator, ItemDetailsCollection)
{
	...
	,	events: {
		...
		,	'click [data-type="add-to-wishlist"]': 'addToWishlist'
        ,	'click [data-type="add-note"]': 'addPersonalNote'
		,	'click': 'contentClick'
		}
	
		, addingWarningDiv: function (type, message) {
			
			//type values: success, info, warning, danger
			
			
			//remove the div if exists				
			if (jQuery("div#warningMsgDiv").length > 0) {
				jQuery("div#warningMsgDiv").remove();
			}

            // dont display div in case of no message
            if(message === '') {
                return;
            }

			jQuery('<div />', {id: 'warningMsgDiv', 'class': 'alert alert-' + type + ' alert-dismissible', role: 'alert', 'style': 'margin-top: 10px'})
				.appendTo('div.add-to-wishlist');
						
			jQuery('<button />', {id: 'warningCloseBtn', type: "button", 'class': "close", 'data-dismiss': "alert"})
				.appendTo('div#warningMsgDiv');

			jQuery('<span />', {id: 'warningSpan', 'aria-hidden': "true"}).appendTo('#warningCloseBtn');
			
			jQuery('<span />', {'class': "sr-only"}).appendTo('#warningCloseBtn');
			
			jQuery('#warningSpan').html("&times;");
			
			jQuery('div#warningMsgDiv').append(message);
		}
        // hide message
        ,   hideMessage: function() {
            this.addingWarningDiv('', '');
        }
		...
		,   addToWishlist: function (e)
		        {
		            ...
		            try {			            
			            var postData = {		            		
			            		siteSettingsId: SC.ENVIRONMENT.siteSettings.id,
			            		customerId: SC._applications.Shopping.user_instance.id,
			            		modelId: this.model.id
			            		};
			            
			            window.addingWarningDiv = this.addingWarningDiv;
			            
			            application.addItemToWishList(postData)
			            	.then(function (resultingData) {
			            		if (resultingData.status === "0") {
		            				window.addingWarningDiv("success", resultingData.message);
		            			} else if (resultingData.status === "1") {
		            				window.addingWarningDiv("info", resultingData.message);
		            			} else {
		            				window.addingWarningDiv("warning", resultingData.message);
		            			}
			            	});
			            console.log("Add to wishlist");
		            } catch (ex) {
		            	//console.log("Some error has occurred! Please try again later.");
		            	window.addingWarningDiv("warning", "Some error has occurred. Please try again later.");
		            }
 		        }
    ,   addPersonalNote: function (e)
    {
        e.preventDefault();
        ...
        if (this.$savingForm.length)
        {
            // Disables add buttons, to prevent double submitions
            this.$savingForm.find('button[id="add-note"]').attr('disabled', true);

            prop = this.$savingForm.serializeObject();
        }
        this.hideMessage();

        prop.siteNumber = SC.ENVIRONMENT.siteSettings.siteid;
        prop.customerId = SC._applications.Shopping.user_instance.id;
        prop.itemId = this.model.id;

        debugger
        if(this.options.personalNoteModel._validate(prop, {validate: true})) {
            var self = this;
            var postData = {
                methodName: 'insertPersonalNote',
                data: prop
            };
            jQuery.ajax({
                url: _.getAbsoluteUrl('services/personalnote.ss'),
                type: 'POST',
                data: JSON.stringify(postData),
                contentType: "application/json; charset=utf-8",
                success: function (data) {
                    //Enables add buttons, to prevent double submitions
                    self.$savingForm.find('button[id="add-note"]').attr('disabled', false);
                    // Clear text area
                    self.$savingForm.find('textarea[id="personalNote"]').val('');
                    // Show Success Message
                    self.addingWarningDiv("success", _('Personal Note added successfully.').translate());
                },
                error: function (ex) {
                    self.$savingForm.find('button[id="add-note"]').attr('disabled', false);
                    // Show Error Message
                    self.addingWarningDiv("danger", ex.message);
                }
            });
        }
        else{
            //Enables add buttons, to prevent double submitions
            this.$savingForm.find('button[id="add-note"]').attr('disabled', false);
            // Show validation error
            this.addingWarningDiv("danger", this.options.personalNoteModel.validationError);
        }
    }
	,	initialize: function (options)
		{
	...
	,	setOption: function (e)
		{
			...
			/*var targetedAnchor = $target[0];
			var selectedColor = '';
			if(!!targetedAnchor) {
				var targetedAnchorLink = jQuery(targetedAnchor).attr("href");
				var linkArray = targetedAnchorLink.split('=');
				if(linkArray.length == 2) {
					selectedColor = linkArray[1];
				}
			}
			if(!!selectedColor) {
				var imgTitle = selectedColor +'.01';
				var isClicked = false;
				jQuery(".bx-pager-item > a > img").each(function() {
					var link = jQuery(this).attr("src");
					if(link.indexOf(imgTitle) > 0) {
						//console.log('exist');
...
define('ItemOptionsHelper', function ()
{
	...
	,	customGetPosibleOptionByCartOptionId: function (cart_option_id)
		{
			return _.where(this.customGetPosibleOptions(), {cartOptionId: cart_option_id})[0];
		}
	...
	,	getPosibleOptions: function () 
		{
			...
			if (this.get('_optionsDetails') && this.get('_optionsDetails').fields)
			{
				...
				_.each(self.get('_optionsDetails').fields, function (option)
				{
	...
	,	customGetPosibleOptions: function () 
		{
			this.cachedPosibleOptions = null;
			if (this.cachedPosibleOptions)
			{
				return this.cachedPosibleOptions;
			}
			...
			if (this.get('_optionsDetails') && this.get('_optionsDetails').fields)
			{
				...
				_.each(fields, function (option_details)
				{
					...
					{
						var item_values = self.get('_matrixChilds').pluck(option.itemOptionId);
						if(!!item_values && item_values.length > 0){
							option.values = _.filter(option.values, function (value)
							{
...
define('SiteSearch', ['Facets.Translator', 'Facets.Model'], function (Translator, Model)
{
	...
	var SiteSearch = {
		...
		searchEventHandler: function (e)
		{
			...
			var resultLength = jQuery('.site-search-container li').length;

			if (resultLength && resultLength <= 2) {
				
				jQuery(e.target).find('input').val("");
				//if there is only one search result returned, along with a see-all, then redirect to it directly
				window.location.href = jQuery('.site-search-container li:nth-child(2) a').attr('href');
			} else {
				//existing logic to redirect to search result page
	...
	,	processAnchorTags: function (e, typeahead)
		{
			...
			var searchSingle = false, singlePath;
			if (typeahead.$menu.find('a').length == 2) {
				searchSingle = true;
				value = jQuery(typeahead.$menu.find('a')[1]).parent().attr('data-value');
				item = typeahead.results[value];

				singlePath = '#' + item.get('_url');	
			}
...
// PersonalNote.js
// -----------------
// Defines the PersonalNote  module (Model)
define('PersonalNote', ['PersonalNote.Model'], function (Model)
{
    'use strict';
